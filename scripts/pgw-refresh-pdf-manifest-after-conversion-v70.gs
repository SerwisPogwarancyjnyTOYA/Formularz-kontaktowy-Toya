/**
 * PGW v70 — PDF Quality Control po konwersji.
 *
 * Co robi:
...
function pgwRefreshPdfManifestAfterConversionV70() {
  const started = new Date();
  const overrides = readRepoJson_(PGW_PDF_QA_V70.repoDataFolderId, PGW_PDF_QA_V70.overridesFileName, {});
  const rows = [];
  const seen = {};

  PGW_PDF_QA_V70.sourceFolderIds.forEach(function(folderId) {
    const folder = DriveApp.getFolderById(folderId);
    scanPdfFolderV70_(folder, folder.getName(), rows, seen, overrides);
  });

  const deduped = dedupePdfRowsV70_(rows);
  ...
  const manifestPayload = {
    ...
    version: '20260709-v70-pdf-quality-control',
    sourceFolderIds: PGW_PDF_QA_V70.sourceFolderIds,
  ...
  upsertRepoJsonV70_(PGW_PDF_QA_V70.repoDataFolderId, PGW_PDF_QA_V70.manifestFileName, manifestPayload);
  upsertRepoJsonV70_(PGW_PDF_QA_V70.repoDataFolderId, PGW_PDF_QA_V70.orphansFileName, orphanPayload);
  upsertRepoJsonV70_(PGW_PDF_QA_V70.repoDataFolderId, PGW_PDF_QA_V70.reportFileName, reportPayload);

  Logger.log('PGW PDF QA v70: scanned=%s visible=%s orphans=%s duplicates=%s overrides=%s',
...
function scanPdfFolderV70_(folder, path, rows, seen, overrides) {
  if (rows.length >= PGW_PDF_QA_V70.maxFilesPerRun) return;

  const files = folder.getFilesByType(MimeType.PDF);
  while (files.hasNext()) {
    if (rows.length >= PGW_PDF_QA_V70.maxFilesPerRun) break;
    ...
    if (!looksLikeAssemblyDrawingPdfV70_(title, path)) continue;

    const detectedModel = extractDeviceIndexV70_(title) || extractDeviceIndexV70_(path);
    const row = buildPdfRowV70_(file, path, detectedModel);
    applyPdfOverrideV70_(row, overrides);
  ...
  while (folders.hasNext()) {
    ...
    scanPdfFolderV70_(child, path + '/' + child.getName(), rows, seen, overrides);
  }
}

function buildPdfRowV70_(file, path, model) {
  const fileId = file.getId();
  const title = file.getName();
  const normalized = normalizeDeviceIndexV70_(model);
  return {
    fileId: fileId,
    title: title,
    fileName: title,
    ...
    keys: buildKeysV70_(title, normalized),
    source: 'POST_CONVERSION_DRIVE_SCAN_V70',
    status: normalized ? 'OK' : 'MODEL_DO_ROZPOZNANIA',
    brand: inferBrandV70_(title + ' ' + path + ' ' + normalized),
    brandConfidence: normalized ? 'filename_or_path' : 'unknown',
    reviewStatus: normalized ? 'visible_after_conversion' : 'needs_model_review'
  };
}

function applyPdfOverrideV70_(row, overrides) {
  const override = findPdfOverrideV70_(row, overrides || {});
  if (!override) return;
  ...
  if (primary) {
    row.deviceIndex = normalizeDeviceIndexV70_(primary);
    ...
    row.keys = buildKeysV70_(row.title, row.deviceIndex).concat(arrayV70_(deviceIndexes).map(normalizeDeviceIndexV70_));
  }
...
function findPdfOverrideV70_(row, overrides) {
  const sources = [
    overrides.byFileId || {},
  ...
  const candidates = uniqueV70_([
    row.fileId, row.fileName, row.title, row.path, row.deviceIndex, row.model,
    normalizeDeviceIndexV70_(row.fileName), normalizeDeviceIndexV70_(row.title),
    normKeyV70_(row.fileName), stripExtV70_(normKeyV70_(row.fileName)),
    normKeyV70_(row.title), stripExtV70_(normKeyV70_(row.title))
  ].filter(Boolean));
  for (let i = 0; i < sources.length; i++) {
    const src = sources[i];
    for (let j = 0; j < candidates.length; j++) {
      const key = candidates[j];
      const variants = uniqueV70_([key, String(key).toUpperCase(), String(key).toLowerCase(), normKeyV70_(key), stripExtV70_(normKeyV70_(key)), normalizeDeviceIndexV70_(key)].filter(Boolean));
...
function dedupePdfRowsV70_(rows) {
  const out = [];
  const byIdentity = {};
  rows.forEach(function(row) {
    const identity = row.fileId || [row.path, row.fileName, row.deviceIndex].map(normKeyV70_).join('|');
    if (!byIdentity[identity]) {
      byIdentity[identity] = row;
      out.push(row);
    } else if (scorePdfRowV70_(row) > scorePdfRowV70_(byIdentity[identity])) {
...
function upsertRepoJsonV70_(repoDataFolderId, fileName, payload) {
  const folder = DriveApp.getFolderById(repoDataFolderId);
...
function looksLikeAssemblyDrawingPdfV70_(title, path) {
  const text = stripDiacriticsV70_((title + ' ' + path).toLowerCase());
...
function extractDeviceIndexV70_(value) {
  const raw = stripDiacriticsV70_(String(value || '')).replace(/\.[Pp][Dd][Ff]$/, '').replace(/^czesci[_ -]zamienne[_ -]*/i, ' ');
...
function normalizeDeviceIndexV70_(value) {
  return extractDeviceIndexV70_(value) || String(value || '').toUpperCase().trim();
}

function buildKeysV70_(title, model) {
  return uniqueV70_([title, stripExtV70_(title), model, normalizeDeviceIndexV70_(model)].filter(Boolean));
}

function inferBrandV70_(text) {
...
function arrayV70_(value) {
  if (Array.isArray(value)) return value;
  if (!value) return [];
  return [value];
}

function stripDiacriticsV70_(value) {
  return String(value || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '');
}

function normKeyV70_(value) {
  return stripDiacriticsV70_(String(value || '').toLowerCase()).replace(/\\/g, '/').split('/').pop().replace(/[_\s]+/g, '-').trim();
}

function stripExtV70_(value) {
  return String(value || '').replace(/\.(pdf|jpg|jpeg|png|webp|docx?|xlsx?)$/i, '');
}

function uniqueV70_(arr) {
  const seen = {};
 ...
 */
function pgwInstallDailyPdfManifestRefreshV70() {
  var triggers = ScriptApp.getProjectTriggers();
  triggers.forEach(function(t) {
    if (t.getHandlerFunction && t.getHandlerFunction() === 'pgwRefreshPdfManifestAfterConversionV70') {
      ScriptApp.deleteTrigger(t);
    }
  });
  ScriptApp.newTrigger('pgwRefreshPdfManifestAfterConversionV70')
    .timeBased()
    .everyDays(1)
    .atHour(4)
    .create();
...
function onOpen() {
  try {
    SpreadsheetApp.getUi()
      .createMenu('PGW PDF')
      .addItem('Odśwież manifest PDF v70', 'pgwRefreshPdfManifestAfterConversionV70')
      .addItem('Zainstaluj dzienny automat v70', 'pgwInstallDailyPdfManifestRefreshV70')
      .addToUi();
  } catch (e) {}
}
