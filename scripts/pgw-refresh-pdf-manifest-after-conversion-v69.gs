/**
 * PGW v69 — PDF Quality Control po konwersji.
 *
 * Co robi:
...
function pgwRefreshPdfManifestAfterConversionV69() {
  const started = new Date();
  const overrides = readRepoJson_(PGW_PDF_QA_V69.repoDataFolderId, PGW_PDF_QA_V69.overridesFileName, {});
  const rows = [];
  const seen = {};

  PGW_PDF_QA_V69.sourceFolderIds.forEach(function(folderId) {
    const folder = DriveApp.getFolderById(folderId);
    scanPdfFolderV69_(folder, folder.getName(), rows, seen, overrides);
  });

  const deduped = dedupePdfRowsV69_(rows);
  ...
  const manifestPayload = {
    ...
    version: '20260709-v69-pdf-quality-control',
    sourceFolderIds: PGW_PDF_QA_V69.sourceFolderIds,
  ...
  upsertRepoJsonV69_(PGW_PDF_QA_V69.repoDataFolderId, PGW_PDF_QA_V69.manifestFileName, manifestPayload);
  upsertRepoJsonV69_(PGW_PDF_QA_V69.repoDataFolderId, PGW_PDF_QA_V69.orphansFileName, orphanPayload);
  upsertRepoJsonV69_(PGW_PDF_QA_V69.repoDataFolderId, PGW_PDF_QA_V69.reportFileName, reportPayload);

  Logger.log('PGW PDF QA v69: scanned=%s visible=%s orphans=%s duplicates=%s overrides=%s',
...
function scanPdfFolderV69_(folder, path, rows, seen, overrides) {
  if (rows.length >= PGW_PDF_QA_V69.maxFilesPerRun) return;

  const files = folder.getFilesByType(MimeType.PDF);
  while (files.hasNext()) {
    if (rows.length >= PGW_PDF_QA_V69.maxFilesPerRun) break;
    ...
    if (!looksLikeAssemblyDrawingPdfV69_(title, path)) continue;

    const detectedModel = extractDeviceIndexV69_(title) || extractDeviceIndexV69_(path);
    const row = buildPdfRowV69_(file, path, detectedModel);
    applyPdfOverrideV69_(row, overrides);
  ...
  while (folders.hasNext()) {
    ...
    scanPdfFolderV69_(child, path + '/' + child.getName(), rows, seen, overrides);
  }
}

function buildPdfRowV69_(file, path, model) {
  const fileId = file.getId();
  const title = file.getName();
  const normalized = normalizeDeviceIndexV69_(model);
  return {
    fileId: fileId,
    title: title,
    fileName: title,
    ...
    keys: buildKeysV69_(title, normalized),
    source: 'POST_CONVERSION_DRIVE_SCAN_V69',
    status: normalized ? 'OK' : 'MODEL_DO_ROZPOZNANIA',
    brand: inferBrandV69_(title + ' ' + path + ' ' + normalized),
    brandConfidence: normalized ? 'filename_or_path' : 'unknown',
    reviewStatus: normalized ? 'visible_after_conversion' : 'needs_model_review'
  };
}

function applyPdfOverrideV69_(row, overrides) {
  const override = findPdfOverrideV69_(row, overrides || {});
  if (!override) return;
  ...
  if (primary) {
    row.deviceIndex = normalizeDeviceIndexV69_(primary);
    ...
    row.keys = buildKeysV69_(row.title, row.deviceIndex).concat(arrayV69_(deviceIndexes).map(normalizeDeviceIndexV69_));
  }
...
function findPdfOverrideV69_(row, overrides) {
  const sources = [
    overrides.byFileId || {},
  ...
  const candidates = uniqueV69_([
    row.fileId, row.fileName, row.title, row.path, row.deviceIndex, row.model,
    normalizeDeviceIndexV69_(row.fileName), normalizeDeviceIndexV69_(row.title),
    normKeyV69_(row.fileName), stripExtV69_(normKeyV69_(row.fileName)),
    normKeyV69_(row.title), stripExtV69_(normKeyV69_(row.title))
  ].filter(Boolean));
  for (let i = 0; i < sources.length; i++) {
    const src = sources[i];
    for (let j = 0; j < candidates.length; j++) {
      const key = candidates[j];
      const variants = uniqueV69_([key, String(key).toUpperCase(), String(key).toLowerCase(), normKeyV69_(key), stripExtV69_(normKeyV69_(key)), normalizeDeviceIndexV69_(key)].filter(Boolean));
...
function dedupePdfRowsV69_(rows) {
  const out = [];
  const byIdentity = {};
  rows.forEach(function(row) {
    const identity = row.fileId || [row.path, row.fileName, row.deviceIndex].map(normKeyV69_).join('|');
    if (!byIdentity[identity]) {
      byIdentity[identity] = row;
      out.push(row);
    } else if (scorePdfRowV69_(row) > scorePdfRowV69_(byIdentity[identity])) {
...
function upsertRepoJsonV69_(repoDataFolderId, fileName, payload) {
  const folder = DriveApp.getFolderById(repoDataFolderId);
...
function looksLikeAssemblyDrawingPdfV69_(title, path) {
  const text = stripDiacriticsV69_((title + ' ' + path).toLowerCase());
...
function extractDeviceIndexV69_(value) {
  const raw = stripDiacriticsV69_(String(value || '')).replace(/\.[Pp][Dd][Ff]$/, '').replace(/^czesci[_ -]zamienne[_ -]*/i, ' ');
...
function normalizeDeviceIndexV69_(value) {
  return extractDeviceIndexV69_(value) || String(value || '').toUpperCase().trim();
}

function buildKeysV69_(title, model) {
  return uniqueV69_([title, stripExtV69_(title), model, normalizeDeviceIndexV69_(model)].filter(Boolean));
}

function inferBrandV69_(text) {
...
function arrayV69_(value) {
  if (Array.isArray(value)) return value;
  if (!value) return [];
  return [value];
}

function stripDiacriticsV69_(value) {
  return String(value || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '');
}

function normKeyV69_(value) {
  return stripDiacriticsV69_(String(value || '').toLowerCase()).replace(/\\/g, '/').split('/').pop().replace(/[_\s]+/g, '-').trim();
}

function stripExtV69_(value) {
  return String(value || '').replace(/\.(pdf|jpg|jpeg|png|webp|docx?|xlsx?)$/i, '');
}

function uniqueV69_(arr) {
  const seen = {};
